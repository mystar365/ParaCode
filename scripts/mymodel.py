...
def gaussian_kernel_density_estimate(x):
    density = kde.evaluate(x.cpu())
...
class MAELoss(nn.Module):
    ...
    def forward(self, input, target):
        loss = torch.sum(torch.abs(input - target))
        return loss
...
class WMAELoss(nn.Module):
    ...
    def forward(self, input, target):
        target_origin = target * lstd + lmean
        target_prob = gaussian_kernel_density_estimate(target_origin)
        target_prob_tensor = torch.from_numpy(target_prob).float().to(device)
        loss = torch.sum((torch.abs(input - target)) * target_prob_tensor)
        return loss
...
class WHuberLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss = torch.where(abs_diff < self.delta,
                           0.5 * (target_prob_tensor * (abs_diff ** 2)),
                           self.delta * (abs_diff * target_prob_tensor) - 0.5 * (self.delta ** 2))
...
class AdaHuberLoss1(nn.Module):
    def __init__(self, delta=0.01, gamma=0):
        super(AdaHuberLoss1, self).__init__()
        self.delta = delta
        self.gamma = gamma
    ...
    def forward(self, input, target):
        ...
        weights = (0.5 - torch.clamp(abs_diff, max=0.5)) ** self.gamma

        loss = torch.where(abs_diff < self.delta, weights * quadratic, weights * linear)    
        
        return torch.sum(loss)
        
        
class WHuberLoss2(nn.Module):
    def __init__(self, delta=0.01):
        super(WHuberLoss2, self).__init__()
        self.delta = delta
    def forward(self, input, target, logg_prob, logg_bias):
        
        weights = logg_prob * (1.1 - logg_bias) 
        abs_diff = torch.abs(input - target)  
...
class WHuberLoss3(nn.Module):
    def __init__(self, delta=0.01):
        super(WHuberLoss3, self).__init__()
        self.delta = delta
